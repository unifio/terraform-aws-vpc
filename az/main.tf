...
locals {
  ...
  azs_provisioned_count = local.azs_provisioned_override_enabled ? length(var.azs_provisioned_override) : var.azs_provisioned
  ...
  eips_enabled_check = var.nat_eips_enabled || var.nat_gateways_enabled ? 1 : 0
  ...
  nat_gateways_enabled_check = var.nat_gateways_enabled ? 1 : 0
...
resource "aws_subnet" "dmz" {
  ...
  availability_zone = local.azs_provisioned_override_enabled ? "${data.aws_region.current.name}${var.azs_provisioned_override[count.index]}" : data.aws_availability_zones.available.names[count.index]
  ...
  cidr_block              = local.dmz_cidrs_override_enabled ? var.dmz_cidrs_override[count.index] : cidrsubnet(
    data.aws_vpc.base.cidr_block,
    lookup(var.az_cidrsubnet_newbits,
    local.azs_provisioned_count),count.index
  )
  ...
  tags = merge(
    ...
    map("Name", "${var.stack_item_label}-dmz-${count.index}")
  )
}
...
resource "aws_instance" "nat" {
  ...
  subnet_id                   = aws_subnet.dmz.*.id[count.index]
  vpc_security_group_ids      = [aws_security_group.sg_nat.*.id[count.index]]
...
resource "aws_security_group" "sg_nat" {
  ...
  ingress {
    cidr_blocks = local.lan_cidrs_override_enabled ? [var.lan_cidrs_override[count.index]] : [cidrsubnet(data.aws_vpc.base.cidr_block,lookup(var.az_cidrsubnet_newbits, local.azs_provisioned_count * local.lans_multiplier),count.index + lookup(var.az_cidrsubnet_offset, local.azs_provisioned_count))]
...
resource "aws_subnet" "lan" {
  ...
  cidr_block = local.lan_cidrs_override_enabled ? var.lan_cidrs_override[count.index] : cidrsubnet(data.aws_vpc.base.cidr_block,lookup(var.az_cidrsubnet_newbits, local.azs_provisioned_count * local.lans_multiplier),count.index + lookup(var.az_cidrsubnet_offset, local.azs_provisioned_count))
  vpc_id     = var.vpc_id
...
resource "aws_route_table_association" "rta_lan" {
  ...
  route_table_id = aws_route_table.rt_lan.*.id[count.index]
  subnet_id      = aws_subnet.lan.*.id[count.index]
}
...
resource "aws_subnet" "static" {
  ...
  # Selects the first N number of AZs available for VPC use in the given region, where N is the requested number of AZs to provision. This order can be overidden by passing in an explicit list of AZ letters to be used.
  availability_zone = local.azs_provisioned_override_enabled ? "${data.aws_region.current.name}${element(var.azs_provisioned_override, count.index)}" : data.aws_availability_zones.available.names[count.index]
  ...
  cidr_block = local.static_cidrs_override_enabled ? var.static_cidrs_override[count.index] : cidrsubnet(
    data.aws_vpc.base.cidr_block,
...
resource "aws_route_table_association" "rta_static" {
  ...
  route_table_id = aws_route_table.rt_static.*.id[count.index]
  subnet_id      = aws_subnet.static.*.id[count.index]
}
